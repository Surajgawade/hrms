...
class Login_model extends CI_Model {
        ...
        function check_password($email,$password){
          ...
          $this->db->select('can_id, can_name,email,password,reporting_to,role_id,can_type,profile_picture,job_profile');
	...
	function check_login_password($email,$password)
   {
      ...
      $this->db->select('can_id, can_name,candidate.email,candidate.password,reporting_to,role_id,can_type,profile_picture,job_profile,login_status');
      $this->db->from('users');
      $this->datatables->join('candidate', 'users.email = candidate.email', 'left');
      $this->db->where($where_arr);
      $query = $this->db->get();
      $num_rows = $query->num_rows();    
      if($num_rows > 0)
