...
<?php foreach($resi_dtls as $dt)
						...
						<tbody>
							...
							<td style="width:15%">
								<?php if($dt->pm_status==1) { ?>
									<a href="<?php echo site_url();?>/resignation/pm_status_update/<?php echo $dt->resi_id; ?>/<?php echo $dt->pm_status; ?>">
							...
							<br />
								...
								<?php if($dt->pm_remark == ""){ ?>
								<span><a data-toggle="modal" href="#myModal" class="remark" data-id="<?php echo $dt->resi_id; ?>" style="text-decoration: none; outline: 0;">Remark</a></span>
								<?php }else{
									echo $dt->pm_remark;
								} ?>
...
<!-- Modal -->
		 ...
		 <div aria-hidden="true" aria-labelledby="myModalLabel" role="dialog" tabindex="-1" id="myModal" class="modal fade">
          	 <form data-toggle="validator" id="resetPassword" name="resetPassword" method="post" action="<?php echo site_url();?>/resignation/pm_remark/" >
  		...
  		<div class="modal-dialog">
      		<div class="modal-content">
          		...
          		<div class="modal-body">
      				...
      				<input type="hidden" id="c_cnt" value="0">
      				<div class="row col-sm-12 resposive" style="overflow: scroll; width: 1000px; max-height:500px;">
...
function get_property(res_id)
{
	$('#resi_id').val(res_id);
	$.ajax({
        url: '<?php echo site_url();?>/resignation/get_marketing_details/'+res_id,
        type: 'POST',
        success: function(response_prop)
        {
        	response_prop = JSON.parse(response_prop);
        	if(response_prop == true)
        	{
				var i = $('#c_cnt').val();
				var data = '<tr id="row-'+i+'"><td><textarea name="rec-client_name-'+i+'" id="rec-client_name-'+i+'"></textarea></td><td><textarea name="rec-client_address-'+i+'" id="rec-client_address-'+i+'"></textarea></td><td><textarea name="rec-	client_contact_no-'+i+'" id="rec-client_contact_no-'+i+'"></textarea></td><td><textarea name="rec-last_dicussion-'+i+'" id="rec-last_dicussion-'+i+'"></textarea></td><td><textarea name="rec-details-'+i+'" id="rec-details-'+i+'"></textarea></td><td><button type="button" class="btn btn-primary" onClick="add_row(\''+i+'\')"><i class="fa fa-plus"></i></button> <button type="button" class="btn btn-danger" onClick="delete_row(\''+i+'\')"><i class="fa fa-minus"></i></button></td></tr>';
				$('#property_data').html(data);
				$('#handoverModal').modal('show');
			}
			else
			{
				$('#save_prop').trigger('click');
			}
		}
	});
}
...
$('#save_prop').on('click', function() {
	...
	$.ajax({
        url: '<?php echo site_url();?>/resignation/save_client_handover/',
        type: 'POST',
        data:fdata,
...
function delete_row(i)
{
	...
	{
		swal("Oops","Cannot delete last row","error");
	}
}
...
{
	i++;
	var data = '<tr id="row-'+i+'"><td><textarea name="rec-client_name-'+i+'" id="rec-client_name-'+i+'"></textarea></td><td><textarea name="rec-client_address-'+i+'" id="rec-client_address-'+i+'"></textarea></td><td><textarea name="rec-	client_contact_no-'+i+'" id="rec-	client_contact_no-'+i+'"></textarea></td><td><textarea name="rec-last_dicussion-'+i+'" id="rec-last_dicussion-'+i+'"></textarea></td><td><textarea type="button" name="rec-details-'+i+'" id="rec-details-'+i+'"></textarea></td><td><button class="btn btn-primary" onClick="add_row(\''+i+'\')"><i class="fa fa-plus"></i></button> <button type="button" class="btn btn-danger" onClick="delete_row(\''+i+'\')"><i class="fa fa-minus"></i></button></td></tr>';
	$('#c_cnt').val(i);
	$('#property_data').append(data);
}
</script>
