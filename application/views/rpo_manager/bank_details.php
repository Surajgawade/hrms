			...
			<form data-toggle="validator" class="col-sm-12" id="frmcan_bank_details" action="" method="post">
				...
				<h1 class="well headline">Bank Details</h1>
					...
					<span class="error_msg" id ="b_name_err"></span>
						...
						<div class="row">
							...
							<div class="col-lg-10">
								...
								<div class="drop-zone">
									...
									<div><span id="uploaded_filename" style="color: #919fa9;"><?php echo (isset($can_bank_details['bank_statement_name']) && !empty($can_bank_details['bank_statement_name'])) ? $can_bank_details['bank_statement_name'] : ''?></span>
						...
						if(isset($can_bank_details['bank_statement_name']) && !empty($can_bank_details['bank_statement_name']) && (file_exists(RPOUPLOADPATH.$can_bank_details['can_id'].'/'.$can_bank_details['bank_statement_name']))){?>
						<div class="row">
							<div class="col-lg-2 col-sm-3 col-xs-12">
								<div class="form-group">
							...
							<div class="col-lg-10 col-sm-9 col-xs-12">
								<div class="form-group">
									<div class="form-control-wrapper form-control-icon-right">
										<a class="btn btn-success" href="<?php echo base_url('uploads/').$can_bank_details['can_id'].'/'.$can_bank_details['bank_statement_name']; ?>" target="_blank">Download</a>
									</div>
								</div>
							</div>
						</div>
...
<script type="text/javascript">
	...
	$('#save_bank_details').click(function (e) {			
		...
		else if($('#IFSC_code').val()=='' || $('#IFSC_code').val().length !=11 || $.isNumeric($('#IFSC_code').val()) == true)
		{
		...
		$.ajax({
			url: '<?php echo site_url();?>/rpo_manager/upload_bank_details',
			data: formData,
			processData: false,
			contentType: false,
			dataType:"json",
