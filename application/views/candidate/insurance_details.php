					...
					<div class="col-sm-12 col-xs-12 profile_bg">
						...
						<div class="row">
							...
							<div class="col-lg-10 col-sm-9 col-xs-12">
								<div class="form-group">
									<div class="form-control-wrapper form-control-icon-right">
										<input type="text" class="form-control" name="joining_date" id="joining_date" placeholder="dd/mm/yyyy" value="<?php echo (isset($can_insurance_details->joining_date) && !empty($can_insurance_details->joining_date)) ? format_date($can_insurance_details->joining_date) : ''?>" required readonly>
									</div>
								</div>
							</div>
						</div>
								...
								<div class="form-group">
									<div class="form-control-wrapper form-control-icon-right">
										<select class="chosen-select web col-lg-12" placeholder="Insurance Company Name" type="text" name="ins_comp_name" id="ins_comp_name" required>
											<?php foreach(@$company_details as $keyc => $valuec) { ?>
												<option value="<?php echo @$valuec['ic_id']; ?>" <?php echo (@$valuec['ic_id'] == @$can_insurance_details->ins_comp_nam) ? "selected" : ""; ?>><?php echo @$valuec['name']; ?></option>
											<?php } ?>
										</select>
					...
					<span class="error_msg" id ="p_no_err"></span>
						...
						<div class="row">
							...
							<div class="col-lg-10 col-sm-9 col-xs-12">
								...
								<span class="error_msg" id ="p_amt_0_err"></span>
									</div>
								</div>
							</div>
						</div>
						...
						<div class="row">
							...
							<div class="col-lg-10 col-sm-9 col-xs-12">
								...
								<span class="error_msg" id ="a_amt_0_err"></span>
									</div>
								</div>
							</div>
						</div>
						...
						<div class="row">
							...
							<div class="col-lg-10">
								...
								<div class="drop-zone">
									...
									<div class="drop-zone-caption">Drag file to upload <strong>(PDF & Doc files only)</strong></div>
									<span class="btn btn-rounded btn-file">
									<span>Choose file</span>
									<input class="file-upload_ins" type="file" id="myfile" name="files" accept="text/plain,.pdf,.doc,.docx/*">
									</span>
...
<script type="text/javascript">
	...
	$('#save_insurance_details').click(function (e) {			
		...
		else if($('#policy_no').val().replace(/\s/g, '')=='')
		{
		...
		else if($('#premium_amnt').val()=='')
		{
			$('#p_amt_err').text(" Please Enter Premium Amount").show().delay(2000).fadeOut(800);
         event.preventDefault();
		}
		else if($('#premium_amnt').val()<=0)
		{
			$('#p_amt_0_err').text("Premium Amount Should Not Accept Only 0's.").show().delay(2000).fadeOut(800);
         event.preventDefault();
		}
		else if($('#assured_amt').val()=='')
		{
			$('#a_amt_err').text(" Please Enter Assured Amount").show().delay(2000).fadeOut(800);
         event.preventDefault();
		}
		else if($('#assured_amt').val()<=0)
		{
			$('#a_amt_0_err').text("Assured Amount Should Not Accept Only 0's.").show().delay(2000).fadeOut(800);
         event.preventDefault();
		}
		else if($('#paid_by').val()=='')
		{
		...
		$.ajax({
			url: '<?php echo site_url();?>/candidate/upload_insurance_details',
			data: formData,
			processData: false,
			contentType: false,
			dataType:"json",
			...
			success: function (response)
			{
				var type='' ;
						...
						{
							type ='danger';
							message ='Insurance date cannot be less than joining date!';
							title ='Joining Date:';
